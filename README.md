...
5. **Protected route requirements** are defined in the [ROUTES](https://github.com/approov/quickstart-nodejs-token-check/blob/refactor/nodejs-quickstart/ApproovApplication.js#L26-L84) via `requiresApproov` and `bindingHeaders`.

6. **Protected routes are registered** in the [ROUTE_TABLE](https://github.com/approov/quickstart-nodejs-token-check/blob/refactor/nodejs-quickstart/ApproovApplication.js#L86-L88) and resolved by the `http.createServer` request handler.
